...
const segments: Segment[] = [
  { outer: 'STACK 1 BLOCK', middle: 'CLIMB 1', color1: '#FFD93D', color2: '#FFF9E3', blocks: 1, climb: 1, icon: '📦' },
  { outer: 'STACK NO BLOCKS!', middle: 'CLIMB 2', color1: '#6BCB77', color2: '#E8F5E9', blocks: 0, climb: 2, icon: '🚫' },
  { outer: 'STACK 1 BLOCK', middle: 'CLIMB 3', color1: '#4D96FF', color2: '#E3F2FD', blocks: 1, climb: 3, icon: '📦' },
  { outer: 'STACK 2 BLOCKS', middle: 'CLIMB 4', color1: '#FF6B6B', color2: '#FFE5E5', blocks: 2, climb: 4, icon: '📦📦' },
  { outer: 'STACK 1 BLOCK', middle: '?', color1: '#95E1D3', color2: '#E8F8F5', blocks: 1, climb: '?', icon: '❓' },
  { outer: 'STACK 1 BLOCK', middle: 'CLIMB 3', color1: '#F38181', color2: '#FFE5E5', blocks: 1, climb: 3, icon: '📦' },
  { outer: 'STACK 1 BLOCK', middle: 'CLIMB 4', color1: '#AA96DA', color2: '#F3EFFF', blocks: 1, climb: 4, icon: '📦' },
  { outer: 'STACK 1 BLOCK', middle: 'CLIMB 2', color1: '#FCBAD3', color2: '#FFF0F5', blocks: 1, climb: 2, icon: '📦' },
  { outer: 'STACK 2 BLOCKS', middle: 'CLIMB 1', color1: '#FFFFD2', color2: '#FFFFEA', blocks: 2, climb: 1, icon: '📦📦' },
  { outer: 'STACK 1 BLOCK', middle: '?', color1: '#A8D8EA', color2: '#E8F6F9', blocks: 1, climb: '?', icon: '❓' },
  { outer: 'STACK 1 BLOCK', middle: 'REVERSE & PLAY', color1: '#FFB6B9', color2: '#FFF0F1', blocks: 1, climb: 'REVERSE', icon: '🔄' },
  { outer: 'STACK 1 BLOCK', middle: 'CLIMB 2', color1: '#FEC8D8', color2: '#FFF5F8', blocks: 1, climb: 2, icon: '📦' },
];
...
function createSegments() {
  ...
  segments.forEach((segment, index) => {
    ...
    textMiddle.style.fontSize = segment.middle === 'REVERSE & PLAY' ? '8px' : '12px';
...
function showResult(segment: Segment) {
  ...
  if (segment.climb === 'REVERSE') {
    html += `<strong>🔄 REVERSE & PLAY!</strong><br>`;
    html += `堆疊 ${segment.blocks} 個方塊<br>`;
    html += `反轉遊戲順序！`;
  } else if (segment.climb === '?') {
    html += `<strong>❓ 神秘箱！</strong><br>`;
    html += `堆疊 ${segment.blocks} 個方塊<br>`;
    html += `抽取特殊卡片！`;
  } else {
